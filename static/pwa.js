(() => {
  ...
  button.type = "button";
  button.textContent = "Preparing Pocketwise…";
  button.setAttribute("aria-label", "Install Pocketwise on this phone");
  Object.assign(button.style, {
    position: "fixed", right: "14px", bottom: "calc(78px + env(safe-area-inset-bottom))",
    zIndex: "9999", display: "block", padding: "11px 15px", border: "1px solid #3bd4a0",
    borderRadius: "999px", background: "#168a68", color: "#fff", fontWeight: "800",
  ...
  window.addEventListener("beforeinstallprompt", event => {
    ...
    promptEvent = event;
    button.textContent = "Install Pocketwise";
  });
  ...
  async function prepare() {
    if (!("serviceWorker" in navigator) || !window.isSecureContext) {
      button.textContent = "Check Pocketwise install";
      return;
    }
    try {
    ...
    setTimeout(() => {
      if (!promptEvent) button.textContent = "Check Pocketwise install";
    }, 1500);
  }
